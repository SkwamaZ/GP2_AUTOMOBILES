...
В репозитории лежат финальные данные, основной ноутбук, код пайплайна и вспомогательные файлы:

- `cars_all.csv` — общий итоговый датасет по всем рынкам.
- `comparison_result.csv` — итоговая таблица сравнения с Россией по моделям.
- `GP_2.ipynb` — основной ноутбук проекта с EDA, проверкой гипотез и бизнес-выводами.
...
- `cars_config.py` — параметры проекта и настройки источников.
- `cars_http.py` — HTTP-обвязка для запросов к внешним источникам.
- `cars_paths.py` — централизованные пути к файлам проекта.
- `cars_logging.py` и `logging.ini` — настройка логирования.
- `cars_normalization.py` и `cars_specs.py` — нормализация названий и характеристик.
- `cars_text.py` — текстовые преобразования и служебные текстовые функции.
- `cars_reporting.py` — таблицы, графики и готовые сводки.
- `russia_cache.csv`, `europe_cache.csv`, `korea_cache.csv`, `asia_cache.csv` — промежуточные кэши по рынкам, которые ускоряют повторную сборку проекта.

Также в проекте есть папка `docs/readme_assets/` с изображениями для README и папка `logs/` с файлами логов.
...
4. После этого можно открыть ноутбук `GP_2.ipynb` и заново пройти весь проект.
...
- `GP_2.ipynb`  
  Для чего: чтобы оформить основной ноутбук проекта более понятно и последовательно.  
